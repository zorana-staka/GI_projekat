...
class Input_file:
    ...
    def read_body_in_file(self):
        ...
        """
        counter = int(self.number_of_threads)
        ...
        with open(self.path_to_idx) as idx_file:
            ...
            split = line.split(':')
            chrom = split[0]
            chrom_pos = split[1].rstrip(';\n')

			
            while counter > 0 and line != "":
                threading.Thread(target = self.read_chrom(chrom, chrom_pos)).start()
                line = idx_file.readline()
                if line != "":
                   split = line.split(':')
                   chrom = split[0]
...
#thread.join()
    ...
    def read_chrom(self,chrom, chrom_pos):
        self.file.seek(int(chrom_pos))
        line = self.file.readline()
        while(line.startswith(str(chrom))):
            body_record_object = Body_record(line, self.body_header_line)
            self.list_of_body_objects.append(body_record_object)
            line = self.file.readline()
